...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message, isOwn, sender, showAuthorName }) => {
    ...
    const bubbleRadius = isOwn ? 'rounded-t-2xl rounded-bl-2xl' : 'rounded-t-2xl rounded-br-2xl'
    ...
    const StatusIcon = ({ status }: { status: string }) => {
        ...
        return (
            <span className={cn('flex items-center relative group', statusInfo.color)} title={statusInfo.tooltip}>
                {statusInfo.icon}
                {/* Tooltip */}
                ...
                <div className="flex flex-col">
                    ...
                    <div
                        className={cn(
                            ...
                            isOwn
                                ? 'flex-row-reverse text-slate-500 dark:text-slate-400'
                ...
                if (msg.isSystem) {
                    return (
                        <div key={msg.id} className="flex items-center justify-center py-2">
                            <div className="px-3 py-1.5 bg-slate-200/50 dark:bg-slate-700/30 rounded-full">
                                <p className="text-xs text-slate-600 dark:text-slate-400 text-center">{msg.text}</p>
                            </div>
                        </div>
                    )
                }
