...
export function Composer({ conversationId, onSend, isSending = false, onFocus }: ComposerProps) {
    ...
    const send = () => {
        ...
        if (!text || isSending || message.length > MAX_MESSAGE_LENGTH) return
                ...
                <div className="flex items-center gap-4">
                    <div className="flex-1 relative">
                        <textarea
                            ...
                            className="w-full bg-slate-100 dark:bg-slate-700 rounded-full py-2 px-4 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm text-slate-800 dark:text-slate-200"
                            rows={1}
                        />
                    </div>
                    <button
